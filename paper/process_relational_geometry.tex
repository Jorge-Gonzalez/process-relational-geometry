...
The primary new mathematical result is a complete isomorphism
classification of single-generator congruences by an invariant
pair $(n, k)$ of period and shift order
(Proposition~\ref{prop:congclass}). The main contribution of the
paper is a \emph{presentation-level semantic framework}: geometric
structure is defined at the level of generators and congruence
laws, prior to quotienting, and metric properties arise as
invariants of congruence classes of processes. This separates
layers --- syntax (generators), laws (congruence), objects
(quotient), and distance (emergent metric) --- that are usually
conflated in classical geometry and in geometric group theory
expositions. The separation makes explicitly askable a question
that classical treatments obscure: what geometric structure does
a specific congruence law produce in the nerve of the free
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Pseudo-periodic.} $a^n \sim \phi$ for some
        $n \geq 1$ and some $\phi \sim a^j$ with $0 < j < n$
        (so $\phi \not\sim \id$, but the orbit revisits a prior
        class). The system neither closes nor runs free.
  \item \textbf{Open.} No periodicity law holds; all powers
        $a^0, a^1, \dots$ are distinct. Quotient: $\mathbb{Z}$.
\end{enumerate}
\end{proposition}

\begin{proof}
By Lemma~\ref{lem:reduction} it suffices to work at minimal
period. Cases (i) and (ii) are distinguished by whether the
return target is $\id$ or some $\phi \not\sim \id$. They cannot
both hold simultaneously. Case (iii) is the negation of (i) and
(ii). The three cases are mutually exclusive and exhaustive.
\end{proof}
...
\begin{definition}[Period and shift order]
\label{def:shiftorder}
Let $(V, R, s, t)$ be a single-generator invertible relational
system with generator $a$ and congruence $\sim$.

\begin{enumerate}[label=(\roman*)]
  \item The \emph{period} $n \in \mathbb{N} \cup \{\infty\}$ is
        the minimal $n > 0$ with $a^n \sim \id$, if it exists,
        and $\infty$ otherwise.
...
\begin{proof}
If $a^m \sim \id$ and $a^p \sim \id$, then $a^{m+p} \sim \id$
by the congruence property. If $a^m \sim \id$, composing on the
right by $a^{-m}$ gives $\id \sim a^{-m}$. Hence $S$ is closed
under addition and negation in $\mathbb{Z}/n\mathbb{Z}$.
\end{proof}

\begin{remark}[Interpretation of $k$]
When $S = \{[0]\}$, no nontrivial power of $a$ is identified with
$\id$: the system is a pure cycle and $k = n/1 = n$.
The case $S = \mathbb{Z}/n\mathbb{Z}$ is unreachable at minimal
period (Lemma~\ref{lem:reduction}). The relevant range is
$1 < k \leq n$, with $k = n$ the closed case and $1 < k < n$
the pseudo-periodic cases. The shift order $k$ equals the index
$[\mathbb{Z}/n\mathbb{Z} : S]$.
\end{remark}

\begin{proposition}[Complete congruence classification]
\label{prop:congclass}
Let $R = \{a, a^{-1}\}$, and let $\sim_1$, $\sim_2$ be congruences
on $\RC$ with invariants $(n_1, k_1)$ and $(n_2, k_2)$. The
quotients $\RC/{\sim_1}$ and $\RC/{\sim_2}$ are isomorphic if
and only if one of the following holds:
\begin{enumerate}[label=(\roman*)]
  \item $n_1 = n_2 = \infty$ (both open; both isomorphic to
        $\mathbb{Z}$);
  \item $n_1 = n_2 < \infty$ and $k_1 = k_2 = n_1$
        (both closed with the same period; $S_1 = S_2 = \{[0]\}$);
  \item $n_1 = n_2 < \infty$ and $1 < k_1 = k_2 < n_1$
        (both pseudo-periodic with the same period and shift order).
\end{enumerate}
No other combination yields isomorphic quotients.
\end{proposition}

\begin{proof}
\textbf{Cases (i) and (ii).} Open congruences yield $\mathbb{Z}$;
closed congruences with period $n$ yield $\mathbb{Z}/n\mathbb{Z}$.
These are isomorphic as monoids iff the periods agree. A finite
monoid is never isomorphic to $\mathbb{Z}$.

\textbf{Case (iii).}
\textit{($\Rightarrow$)} If $\Phi : \RC/{\sim_1} \to \RC/{\sim_2}$
is an isomorphism, it preserves period: $a^{n_1} \sim_1 \id$ implies
$a^{n_1} \sim_2 \id$, giving $n_2 \leq n_1$; by symmetry $n_1 \leq n_2$,
so $n_1 = n_2 =: n$. The isomorphism maps $S_1$ bijectively to $S_2$
(preserving identity and group operation), so $|S_1| = |S_2|$ and
$k_1 = k_2$.

\textit{($\Leftarrow$)} Assume $n_1 = n_2 = n$ and $k_1 = k_2$.
Then $|S_1| = |S_2|$. Since $\mathbb{Z}/n\mathbb{Z}$ is cyclic it
has exactly one subgroup of each order dividing $n$; therefore
$S_1 = S_2 =: S$. Define $\Phi : \RC/{\sim_1} \to \RC/{\sim_2}$ by
$\Phi([a^j]_1) := [a^j]_2$.

\textit{Well-definedness.} If $a^j \sim_1 a^{j'}$, then
$a^{j-j'} \sim_1 \id$, so $[j-j'] \in S_1 = S_2$, hence
$a^{j-j'} \sim_2 \id$ and $a^j \sim_2 a^{j'}$.

\textit{Bijectivity.} $\Psi([a^j]_2) := [a^j]_1$ is the inverse
by the same argument.

\textit{Composition-preservation.}
$\Phi([a^j]_1 \circ [a^m]_1) = [a^{j+m}]_2
= [a^j]_2 \circ [a^m]_2 = \Phi([a^j]_1) \circ \Phi([a^m]_1)$.

\textbf{No other combinations.} Closed quotients (finite, $k = n$)
are not isomorphic to pseudo-periodic quotients (finite, $k < n$),
nor to open quotients (infinite).
\end{proof}
...
isomorphism and is determined by it. The parameter $n$ measures
orbit scale --- how many steps before any return. The parameter
$k$ measures how far the system is from being purely cyclic:
when $k = n$ the system is a pure cycle with no internal loop
structure; as $k$ decreases toward $1$ the internal loop
subgroup $S$ grows, encoding increasingly rich repetition within
each orbit. For multi-generator systems, the natural
generalization is a normal subgroupoid of the free path groupoid;
a complete invariant theory for that case remains open.
\end{remark}

\begin{remark}[Multi-generator classification gap]
\label{rem:multigap}
The framework is universal (Theorem~\ref{thm:reconstruction}):
every finite metric space arises from some multi-generator
relational system. Universality establishes expressiveness but
not classification. The classification problem reduces to
describing normal subgroupoids of free path groupoids up to
isomorphism of quotient categories. Unlike the single-generator
case, no complete invariant analogous to $(n,k)$ is currently
known, even for two generators with full invertibility. The
difficulty is genuine: the corresponding word problem for
finitely presented groupoids is in general undecidable, and the
space of possible congruences is not well-understood even in
simple cases.
\end{remark}
...
The complete isomorphism classification of single-generator
congruences by the invariant pair $(n, k)$
(Proposition~\ref{prop:congclass}) reduces algebraically to
the classification of subgroups of $\mathbb{Z}/n\mathbb{Z}$
by index, which is classical. The specific contribution is the
formulation of this classification in relational-process language
--- characterizing period and shift order as intrinsic invariants
of a congruence on path sequences --- which does not appear in
the existing literature on monoid and group presentations in
this form. The pointed GH emergence theorem
(Theorem~\ref{thm:pointed_gh}) with its three explicit conditions
is a concrete relational formulation not present in the metric
geometry literature. The domain of validity theorem
(Theorem~\ref{thm:scope}), which organizes the boundary between
exactly representable, GH-approximable, and out-of-scope spaces,
is new as a unified statement.
...
torus. What is new is the framing: the presentation-level
perspective makes it possible to isolate the \emph{contribution
of each law} to the homology of the nerve, by comparing the free
and quotient nerves directly. The general question --- which
laws produce which homological changes, and why --- is open
within this framework and appears not to have been studied
systematically in terms of presentation-level nerve comparisons.
