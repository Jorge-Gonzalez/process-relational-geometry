...
Theorem~\ref{thm:reconstruction}, which presents $(X,d)$ with one
generator $e_{xy}$ for each ordered pair $x \neq y$ and
$w(e_{xy}) = d(x,y)$.

\textbf{Preservation under redundant removal.}
We claim that removing a redundant generator $e_{xy}$ leaves the
induced metric unchanged on all pairs. Since $e_{xy}$ is redundant,
there exists a configuration $\gamma^*$ from $x$ to $y$ not using
$e_{xy}$ with $w(\gamma^*) = d(x,y) = w(e_{xy})$. Let $P$ be any
configuration from $u$ to $v$. If $P$ does not use $e_{xy}$, its
weight is unchanged. If $P$ uses $e_{xy}$, substitute each
occurrence of $e_{xy}$ with $\gamma^*$; the resulting path $P'$
satisfies $w(P') = w(P)$ because $w(\gamma^*) = w(e_{xy})$.
Hence every path has an equally-weighted replacement avoiding
$e_{xy}$, so $d_{\mathrm{ind}}(u,v)$ is unchanged for all pairs.

\textbf{Termination.}
Each removal strictly decreases $|R|$, which is finite.

\textbf{Minimality.}
After termination, every remaining generator $e_{xy}$ is
non-redundant: every path from $x$ to $y$ avoiding $e_{xy}$ has
weight strictly greater than $d(x,y)$. Removing $e_{xy}$ would
increase $d_{\mathrm{ind}}(x,y)$, so no further removal is possible
...
\textbf{Full faithfulness.} \textit{Injectivity.} Working in
canonical complete presentations, $R$ contains exactly one generator
$e_{xy}$ for each ordered pair $(x,y) \in V \times V$, and any
morphism $F$ must satisfy $F_R(e_{xy}) = e'_{F_V(x)F_V(y)}$.
Therefore $F_R$ is uniquely determined by $F_V$: two morphisms
agreeing on states must agree on generators.
...
\begin{remark}[Presentation invariant, not group invariant]
\label{rem:presentationinvariant}
The notation $\Delta\mathrm{PD}_n(\langle R \mid \mathcal{L} \rangle)$
is intentional: the argument is the \emph{presentation}, not the
group $G$. This is not a notational convenience; it is a
mathematical necessity.

The depth filtration of Definition~\ref{def:filtration} depends on
two choices that are not intrinsic to $G$:
\begin{enumerate}[label=(\roman*)]
  \item The generating set $R$ and its ordering (which determines
        the shortlex term order orienting the rewriting rules).
        Different orderings of the same $R$ produce different
        normal forms and hence different element depths
        $G_{\leq d}$.
  \item The law lengths $\max(|l|, |r|)$ for $l \sim r \in \mathcal{L}$.
        By Remark~\ref{rem:birthdepth}, every homology class created
        by a law $l \sim r$ has birth depth $\geq \max(|l|, |r|)$.
        A presentation with longer laws produces larger birth
        depths, even if it presents the same group.
  \item The rewriting system used to compute canonical normal forms.
        Different confluent rewriting systems for the same presentation
        can assign different normal forms to the same group element,
        shifting element depths $G_{\leq d}$ and hence birth depths.
        To make $\Delta\mathrm{PD}_n$ a well-defined invariant of the
        pair $(\langle R \mid \mathcal{L} \rangle, \leq_{\mathrm{lex}})$,
        the rewriting system is fixed canonically as the Knuth--Bendix
        completion of $\mathcal{L}$ with respect to the shortlex
        ordering induced by the generator ordering on $R$.
        Under this canonical choice, normal forms are unique and
        the depth filtration is unambiguous.
\end{enumerate}

\textbf{Concrete example.} Consider two presentations of
$\mathbb{Z}^2$:
\begin{align*}
  P_1 &= \langle a, b \;\mid\; ba = ab \rangle
  && \text{(law length } 2\text{)} \\
  P_2 &= \langle a, b \;\mid\; b^2 a^2 = a^2 b^2,\; bab^{-1} = a \rangle
  && \text{(law lengths } 4 \text{ and } 3\text{)}
\end{align*}
Both present $\mathbb{Z}^2$, so $H_2(\mathbb{Z}^2;\mathbb{Z}) \cong \mathbb{Z}$
in both cases. But Theorem~\ref{thm:commutativity_h2} applies to
$P_1$ and gives $\mathrm{birth}_{H_2} = 2$; under $P_2$ the
same class cannot be detected before depth~$3$ (the length of
the shorter law). The group homology is the same; the presentation
persistence diagrams differ.

\textbf{What is a group invariant.} The \emph{stable} output of the
filtration is a group invariant: $H_n(G; \mathbb{Z})$ is recovered
from $H_n(\mathcal{Q}_d)$ for all sufficiently large $d$ (for
finite groups, once $G_{{\leq}d} = G$; for infinite groups, in the
Gromov--Hausdorff limit). The stable value does not depend on the
presentation. What the presentation determines is the \emph{depth
at which} each class first appears --- the birth function --- not
whether the class exists.

\textbf{Consequence for the open question (Q1) of
\Cref{sec:persistent_open}.} Any stability theorem for
$\Delta\mathrm{PD}_n$ (under perturbation of $\mathcal{L}$) must
be stated in terms of a metric on \emph{presentations}, not on
\emph{groups}. The natural metric is the law-length metric: two
presentations $P, P'$ of $G$ are at distance
$\delta(P, P') = \max_{l \sim r \in \mathcal{L} \triangle \mathcal{L}'}
\max(|l|, |r|)$, and the expected stability bound is
$d_{\mathrm{bottleneck}}(\Delta\mathrm{PD}_n(P),
\Delta\mathrm{PD}_n(P')) \leq \delta(P, P')$.
\end{remark}
...
\begin{enumerate}[label=(Q\arabic*)]
  \item \textbf{Stability.} Does the presentation persistence
        ...
        topological data analysis? Since $\Delta\mathrm{PD}_n$
        is a presentation invariant rather than a group invariant
        (Remark~\ref{rem:presentationinvariant}), stability must
        be stated in terms of a metric on presentations.
        The natural candidate is the law-length metric
        $\delta(P, P') = \max_{l \sim r \in \mathcal{L} \triangle
        \mathcal{L}'} \max(|l|, |r|)$, and the conjectured
        stability bound is
        $d_{\mathrm{bottleneck}}(\Delta\mathrm{PD}_n(P),
        \Delta\mathrm{PD}_n(P')) \leq \delta(P, P')$.
        The standard stability theorem for persistent homology of
        filtered simplicial complexes applies~\cite{edels_harer}
        when the interleaving distance between the two filtrations
        is controlled; quantifying that interleaving in terms of
        law-length changes is open.
